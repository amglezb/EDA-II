...
class BTreeNode:
    ...
    def range_search(self, min, max, result): # Busca todos los elementos dentro de un rango.
        i = 0

        while i < self.n and self.keys[i].key < min:
            if not self.leaf and self.C[i]:
                self.C[i].range_search(min, max, result)
            i += 1

        while i < self.n and self.keys[i].key <= max:
            if not self.leaf and self.C[i]:
                self.C[i].range_search(min, max, result)
    ...
    def exact_search(self, key, result): # Busca todos los elementos que coinciden exactamente con una clave.
        i = 0
...
class BTree:
    ...
    def range_search(self, min, max): # Retorna los valores dentro de un rango.
        result = []
        if self.root:
            self.root.range_search(min, max, result)
        return result
...
class FileController:
    ...
    def user_verification(self, user_id, password): # Verifica si el usuario y contraseña coinciden. (Search B-Tree)
...
class System:
    ...
    def edit_txn(self): # Permite modificar campos de una transacción (monto, tipo, categoría, etc.).
        ...
        while True:
            ...
            if self.files.update_txn(trans_id, field, new_value): print(f"{'!!!!! Field updated successfully. !!!!!':^100}")
            else: print(f"{'!!!!! Update failed. !!!!!':^100}")

    def delete_txn(self, trans_id):
        if not self.transaction_tree.search(trans_id): return False
        transactions = self.read_json(self.transactions_file)
        new_data = [t for t in transactions if t["id"] != trans_id]
        if len(new_data) == len(transactions): return False
