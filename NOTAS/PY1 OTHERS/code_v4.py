...
class FileController:
    def __init__(self): # sip
        ...
        self._load_users_btree()
        self._init_files()
        self._load_txns_btree()

    def _load_users_btree(self): # sip
        users = self.load_jsonf(self.users_file)
        for u in users:
            try:
    ...
    def save_jsonf(self, file, data): # sip
        with open(file, "w", encoding="utf-8") as f:
            json.dump(data, f, indent=4)

    def auth_user(self, user_id, password): # sip
        node = self.users_tree.search(user_id)
        if node and node.value.password == password:
            return node.value
        return None

    def next_id(self): # sip
        transactions = self.load_jsonf(self.transactions_file)
        if not transactions: return 1
        return max((t.get("id", 0) for t in transactions), default=0) + 1

    def save_txn(self, transaction): # sip
        transactions = self.load_jsonf(self.transactions_file)
        ...
        self.save_jsonf(self.transactions_file, transactions)
    ...
    def load_user_trees(self, user_id): # sip
        self.amount_tree = BTree(2)
        self.category_tree = BTree(2)
        self.date_tree = BTree(2)
        self.deductible_tree = BTree(2)
    ...
    def find_txn(self, trans_id): # sip
        node = self.transaction_tree.search(trans_id)
        return node.value if node else None

    def update_txn(self, trans_id, field, new_value): # sip
        transactions = self.load_jsonf(self.transactions_file)
        updated = False
        for t in transactions:
...
class System:
    ...
    def login(self, user_id, password): # sip
        user = self.files.auth_user(user_id, password)
        print()
        if user:
            self.current_user = user
            self.files.load_user_trees(user.id)
            print(f"\n\t\t\t!!!!! Welcome {user.name} ¡¡¡¡¡\n")
            return True
        else:
    ...
    def view_txn(self): # sip
        ...
        trans = self.files.find_txn(trans_id)
    ...
    def del_txn_sys(self): # sip
        ...
        if self.files.del_txn_sys(trans_id): print(f"\n\t\t\t!!!!! Transaction #{trans_id} deleted successfully. !!!!!\n")
        else: print("\n\t\t\t!!!!! Transaction not found. !!!!!\n")

    def gen_report(self): # sip
        if not self.current_user:
...
if __name__ == "__main__":
    ...
    while enter:
        ...
        elif op == 4: S.del_txn_sys()
        elif op == 5: S.gen_report()
        elif op == 6: S.calc_isr()
        elif op == 7:
            S.logout()
            break
